...
4. Установить модуль say: `pip install say` (Используется для упрощённой работы с форматированием строк)
5. В `settings.py` заменить `token` на access_token группы. 
6. При необходимости в `settings.py` можно указать (`vk_app_id =`) ID другого приложения вк вместо стандартного app_id из модуля vk_api. 
Значение `-1` указывает на использование app_id по умолчанию.
7. Запустить: `./start.sh` для запуска в screen с циклом перезапуска при краше. 
