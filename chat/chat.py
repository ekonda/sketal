...
2. Добавлять реплики надо через chatter.add(*условие*, *ответ*).
3. Строки рассматриваются, независисмо от регистра!
...
- Compare.inside - True, если вторая строка есть в первой (стока в сообщении пользователя)
- Compare.equals - True, если строки идентичны
- Compare.not_close - True, если строки похожи менее, чем на 80% (с помощью difflib)
- Compare.not_inside - True, если вторая строка отсутствует в первой (стока в сообщении пользователя)
...
** Условия:
Условие - это то, что определяет, будет отвечть на сообщение бот или нет. Условия бывают нескольких видов и
обрабатываются в порядке от первого добавленного до последнего(сверху вниз).

Any(*стока или список строк*, *сравнение*, *реплика поиска*)
