...
@plugin.on_init()
async def init(vk):
    # Объект plugin.data автоматически сохраняется при выключении бота
    # и загружается при старте автоматически. Избегайте крупных объектов
    # Так-же есть plugin.temp_data, которая НЕ сохраняется
